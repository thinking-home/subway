...
1. **Ядро — гранулярность Matter** (стабильный референс); концепты вне Matter — только как явно
   помеченные вендорские расширения (`[VendorExtension]`, см. README ядра).
...
- **Счётчик воды — вендорское расширение ядра (Matter ⊅ концепт), зеркальный случай к AQS.**
  Концепт нужен железу (Waterius) и есть у Алисы (`float:water_meter`, типы `smart_meter.*`), но
  отсутствует в Matter → в ядре появляется помеченный `[VendorExtension]` тип `WaterMeter` и
  свойство `WaterMeterProperty` (м³ — единица ядра совпадает с юнитом Алисы). Назначение канала
  (холодная/горячая) — **монтажная семантика, а не функция железа** (каналы Waterius одинаковы,
  назначение знает установщик), поэтому ей место в метаданных endpoint'а, а не в словаре типов;
  до появления модели метаданных все счётчики **временно** мапятся в `smart_meter.cold_water`,
  потом маппинг типа станет параметризованным (`Cold/Hot → cold_water/hot_water`).
