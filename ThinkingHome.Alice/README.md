...
пайплайн проверяется без секретов.

Изменения **батчируются**: нотифаер копит Report'ы в окне 1 с и шлёт один POST на хост (Яндекс
просит объединять изменения и слать не чаще ~раза в секунду). Склейка — в чистом маппере
(`ToCallbackState`): дедуп по слоту кэша `(устройство, endpoint, instance)` с победой последнего
значения (включая осознанно общий слот цвета), затем группировка по `(устройство, endpoint)` в
`devices[]`. Худшая задержка пуша = окно.
